...
- [ ] `cryostat.agent.discovery.mount-path` [`String`]: the directory path where discovery metadata and hierarchy files are mounted by the Cryostat Operator. The Agent will read JSON files from this location to incorporate Kubernetes context into its discovery information. Default `/tmp/cryostat-agent/discovery`. See [Discovery Integration](docs/discovery-integration.md) for details.
- [ ] `cryostat.agent.discovery.metadata-filename` [`String`]: the filename for the discovery metadata JSON file containing labels and annotations to apply to the Agent's DiscoveryNode and Target. This file should be located within the `cryostat.agent.discovery.mount-path` directory. Default `metadata.json`. See [Discovery Integration](docs/discovery-integration.md) for details.
- [ ] `cryostat.agent.discovery.hierarchy-filename` [`String`]: the filename for the discovery hierarchy JSON file containing the tree structure of DiscoveryNodes (Namespace → Deployment → Pod) that wrap around the Agent's self node. This file should be located within the `cryostat.agent.discovery.mount-path` directory. Default `hierarchy.json`. See [Discovery Integration](docs/discovery-integration.md) for details.
