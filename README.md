...
- [ ] `cryostat.agent.harvester.max-files` [`String`]: the maximum number of files that the agent will keep within the target. The agent will periodically prune, in a FIFO manner, the oldest JFR files within the target's storage, while the number of saved recordings is greater than the maximum file limit. Default `10`.
