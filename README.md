...
- [ ] `cryostat.agent.instance-id` [`String`]: a unique ID for this agent instance. This is published in the target annotations and used by Cryostat to distinguish this agent instance from other agents in the same realm. The default is a random UUID string. It is not recommended to override this value.
...
- [ ] `cryostat.agent.realm` [`String`]: the Cryostat Discovery API "realm" that this agent belongs to. This should be unique per agent instance. The default is the value of `cryostat.agent.app.name` plus a random suffix.
...
- [ ] `cryostat.agent.unified-log.enabled` [`boolean`]: control whether the unified log collection feature is enabled. Default `true`.
- [ ] `cryostat.agent.unified-log.output` [`String`]: the file path that the Agent will configure the JVM to write logs to. If not set or blank, the Agent will generate a temp file path at `/tmp/cryostat-unified-*.log`. Defaults to the empty string.
- [ ] `cryostat.agent.unified-log.output-options` [`String`]: the JVM unified logging `output_options` string controlling log file rotation and retention, e.g. `filecount=10,filesize=100m`. Defaults to `filecount=1000,filesize=200k`.
