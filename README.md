...
- [ ] `cryostat.agent.gc-log.enabled` [`boolean`]: control whether the GC log collection feature is enabled. Default `true`.
- [ ] `cryostat.agent.gc-log.output` [`String`]: the file path that the Agent will configure the JVM to write GC logs to. If not set or blank, the Agent will generate a temp file path at `/tmp/cryostat-gc-*.log`. Defaults to the empty string.
- [ ] `cryostat.agent.gc-log.output-options` [`String`]: the JVM unified logging `output_options` string controlling GC log file rotation and retention, e.g. `filecount=10,filesize=100m`. Defaults to `filecount=1000,filesize=200k`.
